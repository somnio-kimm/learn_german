...
- When verifying the same rule across multiple data cases, consider table-driven tests (`it.each` / `test.each`) first.
- When inputs and outputs cannot be naturally expressed as table-driven tests, it is acceptable to split them into scenario-based tests.
...
- Conversely, are complex object compositions or scenario semantics being forced into table-driven tests (`it.each` / `test.each`) in a way that hurts readability?
...
describe("normalizeName", () => {
  it.each([
    { raw: "  Alice  ", expected: "alice", label: "trim-and-lowercase" },
    { raw: "Bob", expected: "bob", label: "already-normalized" },
  ])("$label: $raw → $expected", ({ raw, expected }) => {
